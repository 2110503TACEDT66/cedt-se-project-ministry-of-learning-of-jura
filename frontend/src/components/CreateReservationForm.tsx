...
export default function ({
  ...
  function parsePeriod(
    periodString: string | null | undefined,
  ): Period | undefined {
  ...
  const formik = useFormik<
    DeepPartial<Omit<Reservation, "reservationDate">> & {
  ...
  async function onRestaurantNameChange(
    ...
    const restaurantsResponse = await fetch(
      `/api/restaurants?name[regex]=${value}&select=name,discounts,reservationPeriods,rooms`,
    ).then((res) => {
      return res.json();
    });
    ...
    const newRoomList = restaurantsResponse.data
  ...
  async function onDiscountChange(
      ...
      >
        ...
        >
          {restaurantsList[0] == formik.values.restaurantName &&
            ...
            periodsList[0].map((period, index) => {
              return (
                <MenuItem
