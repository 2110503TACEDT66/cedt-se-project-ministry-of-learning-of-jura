...
export default function ({
  ...
  return (
    <div className={className ?? ""}>
      <Typography>Id: {reservation._id}</Typography>
      <Typography>Restaurant: {reservation.restaurant?.name}</Typography>
      <Typography>
        Reservation Date:{" "}
        {new Date(Date.parse(reservation.reservationDate)).toLocaleDateString(
          "en-UK",
        )}
      </Typography>
      <Typography>
      ...
      {<Typography>room: {reservation.room ?? "no room"}</Typography>}
